...
// registries, then resolves here. Order for spawn: --team → the open team
// whose lead_session_id is the caller → the team on the caller's own worker
// row (by session_id) → refuse. List uses the same order but falls back to
// project scope instead of refusing.

export const NO_TEAM_MESSAGE = 'no team: pass --team or run golem team lead <team>';
...
export function resolveCallerTeam({ teamRef = null, projectId = null, callerSessionId = null, teams = [], workerRow = null } = {}) {
  ...
  if (caller) {
    const led = open.find((row) => row.lead_session_id === caller);
    if (led) return led;
