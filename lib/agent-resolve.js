...
export function resolveAgentRef(ref, { workers = [], teams = [], projectId = null, callerTeam = null } = {}) {
  ...
  if (bySession.length > 1) {
    // One session on several rows is a stale binding (GOL-382 R6): the live
    // row is the agent; the others are history. Refuse only a real tie.
    const live = bySession.filter(occupied);
    if (live.length === 1) return live[0];
    if (live.length === 0) return bySession[bySession.length - 1];
    throw ambiguous(key, live, teams);
  }
