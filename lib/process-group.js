...
// legacy tmux-row handling (GOL-370 G6/G14). Moved out of the previous host
// driver when managed agents moved to herdr.
//
 ...
 * carry the Pi or Claude executable and the exact --name argument in their
 * command lines; an unrelated recycled group must not match this predicate.
 */
export function processGroupMatches(pgid, { name } = {}) {
  ...
  return rows.some(({ command }) => (
    /(?:^|[\/\s])(?:pi|claude)(?:\s|$)/.test(command) && nameArg.test(command)
  ));
}
