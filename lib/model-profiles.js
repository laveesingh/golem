...
import { getRole, readRoleRegistry, THINKING_LEVELS, validateExecFields } from './session-role.js';
 ...
 *  (model, optional effort); the rule is shared with role presets. */
function validateProfileFields({ harness = 'pi', provider, model, thinking }) {
  return validateExecFields({ harness, provider, model, thinking }, (message) => { throw new Error(`profile ${message}`); });
}

function normalizeProfile(row, index) {
  if (!isRecord(row)) throw new Error(`profiles.json profile #${index} must be an object`);
  try {
    return { name: normalizeProfileName(row.name), ...validateProfileFields(row) };
  } catch (error) {
    throw new Error(`profiles.json profile #${index}: ${error.message}`);
  }
}
...
export function createProfile({ name, harness = 'pi', provider, model, thinking } = {}) {
  const normalized = {
    ...
    ...validateProfileFields({ harness, provider, model, thinking }),
  };
...
export function updateProfile(name, patch = {}) {
  ...
  return mutateStore((store) => {
    ...
    const next = validateProfileFields({
      harness: incoming.harness ?? profile.harness ?? 'pi',
