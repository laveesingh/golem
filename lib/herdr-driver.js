...
/**
 * Ensure the herdr server for `session` is running and headless (U6, verified
 * by the lead): `herdr --session <name> server` started detached, then poll
 * `workspace list` until it answers. An already-running server answers on the
 * first poll and is left untouched.
 */
export async function ensureSession(session, { startupTimeoutMs = 20_000, pollMs = 500 } = {}) {
  ...
  const child = spawn(herdrBinary(), ['--session', resolved, 'server'], {
    detached: true,
  ...
  while (Date.now() < deadline) {
    if (serverReady(resolved)) return { session: resolved, started: true, pid: child.pid };
