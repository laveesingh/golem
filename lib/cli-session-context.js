...
export function resolveCliSessionContext({
  ...
  while (pid > 1 && depth++ < maxDepth) {
    ...
    if (pi) {
      ...
      if (native.procStart && Date.parse(native.procStart) !== process.startedAt) invalid('Claude native process birth identity does not match this PID');
  ...
  const hints = [env.GOLEM_SESSION_ID, env.GOLEM_CEO_SESSION_ID, env.PI_SESSION_ID].filter(Boolean);
  ...
  if (!context && (env.CLAUDE_CODE_SESSION_ID || env.GOLEM_MANAGED_CODEX_BOUND === '1')) invalid('agent context cannot be resolved by this CLI');
