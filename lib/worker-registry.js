...
function nextName(role, projectId, workers, teamId = null) {
  ...
  for (let index = 1; ; index += 1) {
    ...
    if (!nameCollides(candidate, projectId, socket, workers, teamId)) return candidate;
  }
}
 ...
 *
 * Rows that belong to a team (GOL-363 G4) are scoped to the team instead:
 * two teams in one project can each have a builder1, so only an occupied row
 * with the same team_id collides.
 */
function nameCollides(candidate, projectId, socket, workers, teamId = null) {
  if (teamId != null) {
...
export function claimWorker({
  ...
  return withRegistryLock(file, () => {
    ...
    const collision = team != null
      ...
      : registry.workers.find((row) => (
        row.name === workerName && occupied(row)
        && (row.project_id === projectId.trim() || rowSocket(row) === socket)
      ));
    if (collision) {
      const where = team != null ? `team ${team}` : `project ${collision.project_id ?? 'unknown'}, socket ${rowSocket(collision)}`;
