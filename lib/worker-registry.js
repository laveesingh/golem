...
function nextName(role, projectId, workers) {
  for (let index = 1; ; index += 1) {
    ...
    if (!nameCollides(candidate, projectId, workers)) return candidate;
  }
}

/**
 * Name uniqueness stays per project (GOL-370 G7): an occupied row holding the
 * candidate in the same project is a collision, whatever host it runs on.
 */
function nameCollides(candidate, projectId, workers) {
...
export function claimWorker({
  ...
  return withRegistryLock(file, () => {
    ...
    // Name uniqueness stays per project (GOL-370 G7): the herdr session and the
    // herdr agent name are unique within one project.
    const collision = registry.workers.find((row) => (
      row.name === workerName && occupied(row) && row.project_id === projectId.trim()
    ));
    if (collision) {
      throw new Error(`worker name already exists: ${workerName} (project ${collision.project_id ?? 'unknown'})`);
    }
