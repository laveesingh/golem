...
export function notificationReceipt(envelope, retry = null, { includeContent = false } = {}) {
  ...
  const cancelledUnknownAttempt = !!envelope.schedule_id && retry?.status === 'cancelled'
    && !!envelope.delivery_attempt_id && !envelope.accepted_attempt_id && ['pending', 'claimed'].includes(raw);
  let state;
  if (cancelledUnknownAttempt) state = 'uncertain';
  else if (['cancelled', 'expired', 'superseded'].includes(envelope.status)) state = envelope.status;
  ...
  else if (retry?.status === 'cancelled' && envelope.schedule_id) state = 'cancelled';
  ...
  const accepted = raw === 'recovery_required' || cancelledUnknownAttempt ? null
  ...
  const receipt = {
    ...
    ...(envelope.schedule_id ? { schedule_id: envelope.schedule_id, occurrence_seq: envelope.occurrence_seq } : {}),
    ...
    may_have_been_delivered: retry?.status === 'publishing' || !!envelope.delivery_attempt_id
    ...
    next_action: state === 'uncertain' || state === 'blocked' ? 'inspect_before_recovery'
