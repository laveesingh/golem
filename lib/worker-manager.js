 ...
 * `golem agent list` shows what actually runs, not the role default.
 */
 ...
 *
 * G9 team fields ride along additively: team_id, team_label, host (herdr
 * placement or the pre-cutover tmux host) and herdr_state. herdr_state comes
 * from one agent-list fetch per project session per request; pass it in via
 * { herdrStates } (name → state) or leave it null.
 */
export function enrichDispatchableRows(rows, { projectId = null, herdrStates = null } = {}) {
  ...
  return (Array.isArray(rows) ? rows : []).map((row) => {
    ...
    const host = worker == null ? null : (view.herdr_session || view.herdr_workspace_id ? 'herdr' : 'tmux');
    ...
    if (socket !== undefined && hasSession(sessionId, { socket })) {
      try {
        ...
        return {
          ...
          lines,
          text,
          attach_hint: `golem agent attach ${sessionId}${projectId ? ` --project ${projectId}` : ''}`,
          updated_at: new Date().toISOString(),
        };
      } catch (err) {
        return {
