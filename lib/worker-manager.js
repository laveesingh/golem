 ...
 */
export async function ensureProjectWorkspace({ projectId, projectRoot, logFile = null } = {}) {
  const session = herdrSessionForProject(projectId);
  await ensureSession(session, { logFile });
...
export async function spawnWorker({ role, name = null, project = null, cwd = process.cwd(), profile = null } = {}) {
  ...
  try {
    ...
    worker = updateWorker(worker.worker_id, { herdr_agent_name: worker.name });
...
function workerView(worker, roster = null) {
  ...
  return {
    ...
    idle_seconds: idle,
    attach_hint: worker.herdr_agent_name
      ? `golem attach ${worker.herdr_agent_name}`
      : `golem attach ${worker.name}`,
  };
}
  ...
  if (worker) {
    ...
    try {
      ...
      return {
        ...
        lines,
        text,
        attach_hint: `golem attach ${worker.herdr_agent_name ?? worker.name}${worker.project_id ? ` --project ${worker.project_id}` : ''}`,
        updated_at: new Date().toISOString(),
      };
    } catch (err) {
      return {
