...
export function createGolemClient({
  ...
  return Object.freeze({
    ...
    // GOL-369 D5: anchor-targeted block/section reads for both formats.
    getTicketBlockQuery: (id, { block_id = null, anchor = null, section = false } = {}) => {
      const params = {};
      if (block_id != null) params.block_id = String(block_id);
      if (anchor) {
        params.anchor = anchor.text;
        if (anchor.prefix != null) params.prefix = anchor.prefix;
        if (anchor.suffix != null) params.suffix = anchor.suffix;
      }
      if (section) params.section = '1';
      return request('GET', `/api/tickets/${encodeURIComponent(requireNonblank(id, 'getTicketBlockQuery: id'))}/block`, { params });
    },
