...
try {
  ...
  for (const target of ['cc', 'pi']) {
    ...
    if (target === 'cc') {
      assert.ok(rules.includes(INSTRUCTIONS_BEGIN) && rules.includes(INSTRUCTIONS_END), 'cc: instruction markers injected');
      assert.ok(rules.indexOf(INSTRUCTIONS_END) > rules.indexOf(INSTRUCTIONS_BEGIN) + INSTRUCTIONS_BEGIN.length, 'cc: instruction block non-empty');
    } else {
      assert.ok(rules.trim().length > 0, 'pi: rendered instructions non-empty');
    }
    ...
    assert.ok(String(create.description || '').trim().length > 0, 'ticket_create ships a non-empty description');
  ...
  assert.ok(taskTemplate.body.trim().length > 0, 'task template body is non-empty');
  ...
  assert.ok(template.trim().length > 0, 'spec template body is non-empty');
  ...
  assert.ok(promoted.ticket.body.includes(intent), 'staged idea promotion preserves the original intent text');
  assert.ok(promoted.ticket.body.trim().length > 0, 'promoted ticket body is non-empty');
