...
async function forward(method, pathSuffix, body, sessionId, metadata = null) {
  ...
  // current private lease credential.
...
export async function pushBrief(body, sessionId, metadata = null) {
  ...
  // transport receives the correlation id.
...
// A non-ticket control is durable before it is sent. Typed-worker channels
...
export async function pushControlEnvelope({ envelope, content, metadata: suppliedMetadata = null } = {}, sessionId) {
  ...
  // Claude channel gate verdicts ride the legacy /gates/... routes; the generic
  // /brief path covers everything else.
  if (!legacy?.path) return { ok: false, status: 400, body: '', error: 'legacy control route is required for this channel kind' };
